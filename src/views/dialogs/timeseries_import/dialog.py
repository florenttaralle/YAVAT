...
class TimeseriesImportDialog(QDialog):
    def __init__(self, df: pd.DataFrame, yavat: YavatModel, parent: QWidget|None=None):
        ...
        self._df    = df
        self._yavat = yavat
        self.setWindowTitle("Timeseries Import")
        self.setModal(True)
        ...
        <html>
            ...
            You can import multiple timeseries at once. <br>
    ...
    def _get_columns(self):
        x_columns = [
            ...
            if column.cfunction.is_timestamp
        ]
        y_columns = [
            column
            for column in self._columns._columns
            if column.used and not column.cfunction.is_timestamp            
        ]
    ...
    def _update(self):
        ...
        valid = True
        if len(x_cols) < 1:
            self._msgs.appendRow(QStandardItem(Icons.MessageWarning.icon(), "No FrameId or Timestamp selected."))
            valid = False
        elif len(x_cols) > 1:
            self._msgs.appendRow(QStandardItem(Icons.MessageWarning.icon(), "More than one FrameId or Timestamp selected."))
            valid = False
        if len(y_cols) < 1:
    ...
    def exec(self) -> bool:
        ...
        if status == QDialog.DialogCode.Accepted:
            ...
            if self._yavat is None:
                ...
                x_values        = self._format_x_values(x_cols[0])
                for column in y_cols:
