...
from PyQt6.QtCore import pyqtSignal, Qt, QRectF, QLineF, QPointF, QEvent
from PyQt6.QtGui import QKeyEvent, QPainter, QPen, QColorConstants
...
from src.models.annotation import EventModel
...
class EventView(QGraphicsObject):
    ...
    def mouseMoveEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        match self._state:
            case EventViewState.MovingBody:
                offset_x     = event.pos().x() - self._press_x
                closest_x0   = round(self._event.first + offset_x)
                closest_x1   = round(self._event.last + offset_x)
                if (closest_x0 >= self._event.first_min) and (closest_x1 <= self._event.last_max) and (closest_x0 != self._event.first):
                    self._press_x += closest_x0 - self._event.first
                    self._event.set_first(closest_x0)
                    self._event.set_last(closest_x1)

            case EventViewState.MovingLeftHdl:
                closest_frame_id = round(event.pos().x())
                if (closest_frame_id >= self._event.first_min) and (closest_frame_id <= self._event.first_max):
                    self._event.set_first(closest_frame_id)
            
            case EventViewState.MovingRightHdl:
                closest_frame_id = round(event.pos().x())
                if (closest_frame_id >= self._event.last_min) and (closest_frame_id <= self._event.last_max):
                    self._event.set_last(closest_frame_id)
        event.accept()
