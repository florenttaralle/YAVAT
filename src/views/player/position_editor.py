...
class TimeValidator(QValidator):
    ...
    def __init__(self, max_value: QTime, format: str="mm:ss:zzz", parent: QObject|None = None):
    ...
    def validate(self, input: str, pos: int) -> QValidator.State:
        ...
        if value.isValid() and (QTime(0, 0, 0, 0) <= value < self._max_value):
            state = QValidator.State.Acceptable
        return state, input, pos

class PositionEditorView(AutoPauseLineEdit):
